...
ตอบ main/main.c CMakeLists.txt sdkconfig idf.py build/
2. ความแตกต่างระหว่าง `hello_esp32.bin` และ `hello_esp32.elf` คืออะไร?
ตอบ hello_esp32.elf ไฟล์ที่คอมไพล์แล้วพร้อม สัญลักษณ์ debug (ใช้ดูโค้ด, ตัวแปรใน GDB)
    hello_esp32.bin ไฟล์ ไบนารีบริสุทธิ์ สำหรับแฟลชลงชิปจริง (ไม่มีข้อมูล debug)
3. คำสั่ง `idf.py set-target` ทำอะไร?
ตอบ ช้เพื่อ ตั้งค่าประเภทชิป ที่จะ build โค้ดให้เข้ากัน เมื่อสั่งคำสั่งนี้ ระบบจะสร้างไฟล์ config และ toolchain ที่เหมาะกับชิปนั้นโดยเฉพาะ
4. โฟลเดอร์ `build/` มีไฟล์อะไรบ้าง?
ตอบ .bin → ไฟล์ไบนารีที่ใช้ flash
    .elf → ไฟล์สำหรับ debug
    .map → แผนผังหน่วยความจำของโปรแกรม
    bootloader/ → โค้ด bootloader ที่จะรันก่อนโปรแกรมหลัก
    esp-idf/ → ไฟล์ไลบรารีของ ESP-IDF ที่ build แล้ว
    CMakeCache.txt, compile_commands.json → ข้อมูลการ build
5. การใช้ `vTaskDelay()` แทน `delay()` มีความสำคัญอย่างไร?
ตอบ vTaskDelay() → เหมาะกับระบบ Multitasking 
    delay() → ทำให้ระบบค้าง 
